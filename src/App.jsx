...
// ── FIXED loadCloset — Supabase FIRST, localStorage only as fallback ──────────
...
const plans=[
{id:"free",name:"Free",monthly:0,yearly:0,badge:"",features:["Up to 20 closet items","3 outfit generations per week","AI styling + smart filtering","AI photo recognition","Style profile + AI learns your taste","Saved favourite outfits","Mood Board + On Body mannequin view","Share outfits with friends","Store suggestions","Friends voting on your outfits"],current:!user.pro},
{id:"pro",name:"Pro",monthly:6.99,yearly:4.99,badge:"Most Popular",features:["Everything in Free — unlimited","Unlimited closet items","Unlimited outfit generations","🧳 Trip Packer","🎉 Event Planner","🔔 Sale alerts on suggested items","📅 Outfit wear history","📸 Photo outfit matching","🗑️ Closet Cleanup — clear out unworn items","👗 Virtual Try-On ✦ Coming Soon","📸 AI Editorial Photos ✦ Coming Soon","🌍 Trend Awareness ✦ Coming Soon"],current:user.pro},
];
return (
              ...
              <div style={{border: "2px dashed " + (newItem.image ? G : BORDER), borderRadius: 12, overflow: "hidden", transition: "border-color 0.2s", position: "relative"}}>
                {!newItem.image && !aiScanning && (
                  <div style={{padding:"20px 16px"}}>
                    ...
                    <div style={{display:"flex",gap:8}}>
                      <button onClick={()=>cameraRef.current.click()} style={{flex:1,padding:"10px 8px",background:"#111009",border:"1px solid "+BORDER,borderRadius:9,cursor:"pointer",fontFamily:"'DM Mono'",fontSize:11,color:"#AAA",display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
                        <span style={{fontSize:18}}>📸</span><span>Take Photo</span>
                      </button>
                      <button onClick={()=>fileRef.current.click()} style={{flex:1,padding:"10px 8px",background:"#111009",border:"1px solid "+BORDER,borderRadius:9,cursor:"pointer",fontFamily:"'DM Mono'",fontSize:11,color:"#AAA",display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
                        <span style={{fontSize:18}}>🖼</span><span>Upload File</span>
                      </button>
                    </div>
                  </div>
                )}
...
// ── FIXED: Check for saved user on mount, but always load data from Supabase first ──
useEffect(()=>{
(async()=>{
  let loggedInUser = null;
  try {
    const u = localStorage.getItem("clozie-user");
  ...
  if (loggedInUser) {
    ...
    setPage("app");
    // Load data using the logged-in user's email as the key
    const uid = loggedInUser.email;
...
setUser(u);
// Load THIS user's data from Supabase immediately on login
(async()=>{
  const uid = u.email;
  const sc = await loadCloset(uid);
  const sp = await storageGet("clozie-profile", uid);
  const sl = await storageGet("clozie-learnings", uid);
  const sf = await storageGet("clozie-favs", uid);
  setCloset(sc&&Array.isArray(sc)?sc:[]);
  setProfile(sp&&sp.styles?sp:{styles:[],colors:[],dislikes:""});
  setLearnings(sl&&Array.isArray(sl)?sl:[]);
  setFavOutfits(sf&&Array.isArray(sf)?sf:[]);
  setReady(true);
})();
setPage("app");
};
...
<MainApp
user={user}
onLogout={()=>{try{localStorage.removeItem("clozie-user");}catch(e){}setUser(null);setPage("welcome");setCloset([]);setProfile({styles:[],colors:[],dislikes:""});setLearnings([]);setFavOutfits([]);}}
...
<Settings user={user} onBack={()=>setPage("app")} onLogout={()=>{try{localStorage.removeItem("clozie-user");}catch(e){}setUser(null);setPage("welcome");setCloset([]);setProfile({styles:[],colors:[],dislikes:""});setLearnings([]);setFavOutfits([]);}} onSubscription={()=>setPage("subscription")}/>
</div>
)}
{page==="subscription"&&(
<div style={{position:"fixed",inset:0,zIndex:500,overflowY:"auto",background:BG}}>
