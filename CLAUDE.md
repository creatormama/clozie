...
Last updated: **2026-08-03 — Update 4 — Session 28.** Background removal measured in isolation (Mac harness, AWB off, enhance 0) — the cutout pipeline is colour-clean (mask / crop / composite / PNG add no colour shift) and NO fix is validated; full detail in SESSION_NOTES Update 4 — Session 28. Standing constraints: **Build 29 / v1.0.5 is LIVE**; the Session 26 `MEASURE_COLOR_MODE` flag stays in code and MUST be removed before any App Store promotion; main `062d15b` / production `0baff39` untouched, cache 2,510. Next session: test correction candidates in the Mac harness against the 4-photo set — Grace's eyes are the only pass gate.
