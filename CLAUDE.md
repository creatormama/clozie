...
- Current Expo SDK: 57 (`expo` ^57.0.0, installed 57.0.4 / `react-native` 0.86.0 — VERIFIED from package.json + node_modules 2026-07-21; the SDK 54→57 upgrade landed Update 3 — Session 9). Do NOT run `npm audit fix`. The ban originated in the SDK-54 era (npm audit fix broke the Metro bundler; it was deferred TO the SDK upgrade) and it is STILL IN FORCE. It has NOT been re-validated against SDK 57 — whether `npm audit fix` is now safe on 57 has never been tested by anyone. Untested ≠ cleared: treat it as banned until someone actually tests it. See THINGS TRIED THAT DID NOT WORK.
...
Last updated: **2026-08-03 — Update 4 — Session 28 (READ-ONLY — background removal measured IN ISOLATION; pipeline is colour-CLEAN).** Session 27's Step 2 executed: a Mac swiftc harness ran the REAL cutout (Vision mask + faithful copy of `removeBackground()`) on 4 raw `fixed.uri` captures (warm white / daylight white / oatmeal / baby blue) with `autoWhiteBalance` OFF and `enhanceStrength` 0, producing BOTH endings — PATH A (composite over white → JPEG, Build-25 look) and PATH B (transparent PNG, current look). **MEASURED: the pipeline adds NO colour shift** — garment-interior median RGB is identical (max delta 0–1 = rounding) across Source → Post-mask → Path A → Path B on every photo; the Display-P3→sRGB output conversion is properly colour-managed and correctly tagged sRGB (NO profile mismatch). **So the mask / crop / white-composite / PNG-encode did NOT brown the whites — the cutout machinery is innocent.** Also MEASURED: source photos carry a different cast per lighting (same white tee R−B **+33** warm-light vs **−49** daylight — the cast is in the CAPTURE, not the pipeline). PRIOR-established, restated (NOT this session's harness): enhance damaged Build-26-era stored files (Build 26 TestFlight, 2026-07-14 Known Issue); current AWB washes pales in the live closet (Session 27, `sC=1.00`). **NO fix built or validated — Session 27's Direction B failed Grace's eyeball (oatmeal too yellow).** 4 side-by-side renders delivered; Grace's eyes remain the only pass gate. NEXT SESSION: test correction candidates in the Mac harness against this 4-photo set. Commits (testing, named-file only): this pointer + the SESSION_NOTES entry. **testing ahead of origin, NOT pushed. main `062d15b` / production `0baff39` UNTOUCHED. No Edge Function / SYSTEM_PROMPT changes; cache 2,510. Build 29 / v1.0.5 still LIVE. The Session 26 `MEASURE_COLOR_MODE` flag stays in code and MUST be removed before any App Store promotion.** Full detail: SESSION_NOTES Update 4 — Session 28.

**2026-08-03 — Update 4 — Session 27 (READ-ONLY Mac measurement — AWB washout root cause CONFIRMED in code; scene-reference fix simulated; 4 visual comparisons rendered; root-cause investigation queued).** No shipping code, no build, no Edge Function — all work in the scratchpad (outside the repo); `AutoWhiteBalance.swift` + `BackgroundRemovalModule.swift` READ only. Instrumented Mac harnesses (real Vision mask + verbatim AWB math) CONFIRMED the Session 26 diagnosis at the code level: the AWB estimator reads a pale garment's OWN colour as the "illuminant" (oatmeal→warm, baby blue→cool), neutralises it and lifts ~1.5× toward white, while the chroma-protection gate NEVER fires (`sC = 1.00` on every pale; meanBodyChroma 0.05–0.15 ≪ protLo 0.22). Harness proven faithful (raw + corrected RGB match Grace's sips; oatmeal warmth R−B 43→4 = the washout). A scene-reference fix was simulated: **white-patch on the background reads ≈neutral in every shot (gray-world is defeated by the red rug), so it PRESERVES oatmeal (247,235,203) and baby blue (195,222,232)** where current AWB washes them out — BUT the SAME white tee does NOT converge across lights (warm→cream R/B 1.27, daylight→blue 0.63; a beige couch under cool light read falsely-neutral), and current AWB converges whites better. 4 visual comparisons (RAW | CURRENT | DIR-B) rendered in scratchpad. **Grace's eyeball verdict (a REQUIRED gate — numbers alone are not enough): Direction-B oatmeal reads too YELLOW; blue + white acceptable.** **Hybrid B (scene as CLASSIFIER — near-whites get current AWB, colours protected) is the LEAD fix candidate** (keeps the Session 13–19 white-fix AND preserves pales; fails only false-neutral scenes); Hybrid A (chroma threshold) is the low-risk fallback (wide gray zone). **ROOT-CAUSE PIVOT (Grace): whites WERE white when background removal first shipped and turned beige/brown LATER — so the true root is a CHANGE in between (BR-module rework / SDK 54→57 / capture-resize / card compositing), none assumed. No more patches on patches; colours must not disagree (same garment → same answer).** NEXT SESSION, in order: (a) timeline from notes + build history to find which change browned the whites; (b) measure background-removal IN ISOLATION (same image in vs out, `autoWhiteBalance` OFF) — never done before; (c) then decide the fix from the root, Hybrid B lead / A fallback, validated via V3. Commits (testing, named-file only): this SESSION_NOTES entry + this pointer. **testing @ `dfea4b8`, ahead of origin, NOT pushed. main `062d15b` / production `0baff39` UNTOUCHED. No Edge Function / SYSTEM_PROMPT changes; cache 2,510. Build 29 / v1.0.5 still LIVE. The Session 26 `MEASURE_COLOR_MODE` flag stays in code and MUST be removed before any App Store promotion.** Full detail: SESSION_NOTES Update 4 — Session 27.

**2026-08-02 — Update 4 — Session 26 (measure-only Build 30 + DECISIVE measurement — washout root cause FOUND, capture-path theory OVERTURNED).** Built a measure-only TestFlight build (v1.0.6 / Build 30) that shares the raw `fixed.uri` from BOTH the camera and library Add-Item paths (temporary `MEASURE_COLOR_MODE` flag, purely additive, TestFlight-only). Grace AirDropped the same oatmeal cardigan captured 3 ways; measured on Mac (sips + PIL): raw CAMERA garment = **(211,191,164)** and raw LIBRARY = **(218,198,168)** — both TRUE warm oatmeal, nearly identical; the DISPLAYED closet card = **(227,225,223)** — near-white, warmth (R−B) collapsed from ~47 to ~4, saturation ~46 → ~3.8. **So the visible washout is ADDED AFTER capture by the background-removal `autoWhiteBalance` step (Fork-A AWB white-fix), NOT the capture path.** This OVERTURNS the Session 25 capture-path color-profile theory for the displayed washout. Also confirmed: all 4 measured captures carried a **byte-identical Display P3 ICC profile** (same MD5) — NO tag mismatch; a P3→sRGB capture conversion would have done nothing and risked dulling real colours — **measuring first avoided shipping the wrong fix.** NEXT STEP (Mac, NO build): test `autoWhiteBalance` OFF vs ON on the oatmeal cutout, **guarded against a WHITES regression** — AWB was added by the Session 13–19 white-fix work to clean whites (`AutoWhiteBalance.swift`, app-code commit `5b51910`, Session 19), and Grace specifically recalls that raising whites made "oatmeal too white"; review where the white-fix was introduced and what it solved BEFORE changing it. The real fix likely must separate "warm CAST to neutralize" (whites) from "genuine warm garment COLOUR to keep" (oatmeal), not a blanket AWB toggle. Commits (testing, named-file only): version bump `64b980e` (1.0.5→1.0.6), measure `17472d1` (`App.js`, +20 lines, purely additive). Restore tag `v2026-08-02-pre-camera-color-fix` @ `c447731`. **testing @ `17472d1`, ahead of origin by 4, NOT pushed. main `062d15b` / production `0baff39` UNTOUCHED. No Edge Function / SYSTEM_PROMPT changes; cache 2,510. Build 29 / v1.0.5 still LIVE; Build 30 measure-only, NOT promoted. The measure flag stays in code and MUST be removed in the fix build before any App Store promotion.** Full detail: SESSION_NOTES Update 4 — Session 26.

**[SUPERSEDED 2026-08-02 — the DISPLAYED washout is background-removal AWB over-whitening a warm garment, NOT the capture path; see the Session 26 block above. All 4 measured captures were byte-identical Display P3 (no tag mismatch), so this block's capture-path diagnosis is RETIRED. Kept for the record.]**
**2026-08-01 — Update 4 — Session 25 (capture-path color-profile diagnosis — READ-ONLY).** The oatmeal / pale-warm "washout" is now diagnosed as a **CAPTURE-PATH color-profile bug, NOT AWB and NOT lighting.** Grace's controlled test (same cardigan): camera-roll import = correct ("Oatmeal"); in-app camera daylight = washed ("Stone Grey"); warm light = washed ("Powder Blue") → capture path predicts it, not lighting. VERIFIED in code: both Add-Item paths are byte-identical after acquisition; the only difference is the picker — the in-app camera (`UIImagePickerController`, no original file) is forced to redraw + re-encode via `jpegData` (dropping/mistagging the Display-P3 profile), while the library path (`PHPickerViewController`) passes the original HEIC bytes through untouched. Recognition runs on `fixed.uri` BEFORE any AWB yet still comes back washed → the washout is UPSTREAM of AWB, so **the previously-scoped AWB brightGain luma-gate is RETIRED for this issue.** expo-image-picker / expo-image-manipulator `57.0.2` expose NO color-space flag → **new fix direction: a COLOR-MANAGED P3→sRGB conversion at the CAPTURE stage, upstream of the split — fixes both the display washout AND the color-naming error at their shared root (NOT a blind re-tag, which would dull colors).** Not yet designed/built; must not regress background removal, whites, or colored clothes (Grace's hard constraint). Full detail + build plan + regression checklist: SESSION_NOTES Update 4 — Session 25. No code / build / Edge Function this session; main `062d15b` / production `0baff39` UNTOUCHED; testing HEAD advanced by docs commits only.

**2026-07-29 — Update 4 — Session 24 (closet blank-images diagnosis + 1e cold-weather warmth interim).** Two things. (1) **1e COLD-WEATHER WARMTH INTERIM — DONE + DEPLOYED:** new `RAIN_OUTERWEAR` const + a filter in `applySafetyFilters` (right after C4 Snowy) drops thin rain shells (rain jacket/coat, anorak, windbreaker) in Cold/Snowy, KEEPS them in Cold+Rainy (carve-out); pinned-exempt; SYSTEM_PROMPT untouched, cache stays 2,510; deployed via CLI `--use-api` → `generate-outfits` now live **Version 63 (ACTIVE)**; on-device + Supabase Logs verified. (Edge version was actually **62** pre-deploy — the older "61" note was one version stale.) Commit `b0a6621` on `testing` (index.ts only, not pushed). (2) **CLOSET BLANK-IMAGES BUG — diagnosed, fix banked, DEFERRED to Aug 1:** closet images are private Supabase signed URLs (1h TTL) minted once on mount and never re-minted on foreground return, so >1h background → blank images (force-quit fixes it). Option A fix (gated foreground re-mint via the safe functional-map setter) fully spec'd + banked to `Clozie_ClosetImage_SignedURL_Fix_OptionA_Spec_2026-07-29.md` (commit `bc80ca5`); it's an App.js native change, blocked until the EAS quota resets Aug 1. **NEXT OPEN ITEM — warmth-data layer:** the rain-only interim does NOT cover light non-warm outerwear (denim/leather jackets) or light/short-sleeve tops still appearing in Cold/Snowy; the real fix is tagging items light/mid/warm then requiring warm-enough pieces — NOT an ever-growing name blocklist. main `062d15b` / production `0baff39` UNTOUCHED; nothing pushed this session.

**2026-07-27 — Update 4 — Session 23 (Edge Function only: Issue D new-item "*" bias DISABLED — reversible, cache-safe). NO app change, NO EAS build.** The `todayMark` `* ` generation in `buildCompressedPool` (USER message) is now gated behind a new module-level `const NEW_ITEM_BIAS_ENABLED = false` in `supabase/functions/generate-outfits/index.ts` — with the flag false, no item created today gets a `*`; flip to `true` to restore. Rationale: the `*` fired for ANY item created today, so Grace's July-20 bulk re-photography of the whole closet tripped it across everything — the code can't tell "bought for tonight" from "just re-shot my closet"; pinning is the deliberate replacement. Code KEPT, not deleted — `isToday`, the `todayMark` logic, the interpolation site, and the `createdAt` newest-first sort are all untouched and reversible. **SYSTEM_PROMPT untouched — byte-identical (9,266 bytes / 35 em-dashes / 0 middots); cache VERIFIED 2,510 via round-trip (cache_creation 2510 → cache_read 2510, 15s apart, Supabase Logs).** Rule 12 + anchor-priority-5 in SYSTEM_PROMPT are now DORMANT (no `*` ever emitted) but deliberately NOT stripped — removing them is a cache reset + byte audit, deferred to a batched edit. One CLI deploy (`--use-api`, no `--yes`); flag confirmed LIVE at the edge via download-and-grep (`NEW_ITEM_BIAS_ENABLED = false` + the gated `todayMark` line both present in the deployed source). Commit `4dfc54b` on `testing` (index.ts only, not pushed). App.js NOT touched. main `062d15b` / production `0baff39` UNTOUCHED. Edge Function `generate-outfits` generating normally (source: sonnet, 3 outfits).

**2026-07-21 — Update 4, Session 22 (Build 29 App Store release + git bookkeeping — production intentionally moved).** **Build 29 (v1.0.5) is now the LIVE App Store build** — Apple-approved; Grace pressed Release in App Store Connect 2026-07-21 (US, 1 region; rollout in progress). **Build 25 / v1.0.4 → prior live build**; its restore tag `v1.0.4-build25-appstore-live` (tag-object `d219721`) @ `f711c5d` stays permanent (VERIFIED intact after the move). **The v1.0.5 train is now CLOSED** — per the VERSION RULE the next app CODE change bumps the version to 1.0.6 in BOTH `app.config.js` AND `package.json` first; both files read `1.0.5` on disk right now (VERIFIED, no bump yet). **Release git bookkeeping DONE 2026-07-21:** annotated tag `v1.0.5-build29-appstore-live` (tag-object `e2b79f2`) created on the shipped Build 29 commit `0baff39` AND `production` fast-forwarded `f711c5d` → `0baff39` (true fast-forward, 41 commits, nothing lost); both pushed to origin (local `production` = `origin/production` = `0baff39`, VERIFIED). EAS server record confirms Build 29 = commit `0baff39` (`eas build:view`: Version 1.0.5 / Build 29 / Commit `0baff3924ec…`). The descriptive restore tag `v1.0.5-build29-awb-whitefix` @ `0baff39` also remains. AWB phase15D detail unchanged — SESSION_NOTES Update 4 — Sessions 13–19. Two known AWB colour limitations (dim-warm whites not fully white; pale-warm/oatmeal over-whitens in bright light) + one out-of-scope busy-carpet cutout miss remain — see KNOWN ISSUES / `Clozie_Known_Issues_Backlog.md`. **main `062d15b` UNTOUCHED this session** (never checked out, never moved); cache 2,510 (nothing deployed); zero Edge Function / SYSTEM_PROMPT / app-code changes; **EAS quota: 0 builds remain this month.**
...
- Pale WARM garment washout (oatmeal / cream reads near-white on the closet card) — ROOT CAUSE FOUND + MEASURED 2026-08-02 (Session 26). The background-removal `autoWhiteBalance` step (Fork-A AWB white-fix, `modules/expo-background-removal/ios/AutoWhiteBalance.swift`, app-code commit `5b51910`, Session 19) over-whitens near-neutral WARM garments toward white. Measured (same cardigan): raw camera `fixed.uri` (211,191,164) and raw library (218,198,168) are both true warm oatmeal and nearly identical; the DISPLAYED card is (227,225,223) — near-white, warmth R−B collapsed ~47→~4, saturation ~46→~3.8. **The Session 25 "capture-path color-profile" theory is RETIRED for the displayed washout** — all four measured captures were byte-identical Display P3 (no tag mismatch), so a P3→sRGB capture convert would have done nothing. NEXT STEP (Mac, no build): compare the cutout with `autoWhiteBalance` OFF vs ON. **HARD GUARD: must NOT regress WHITES** — AWB was added by the Session 13–19 white-fix to clean whites, and raising whites is what made oatmeal too white; the real fix must separate "warm CAST to neutralize" (whites) from "genuine warm garment COLOUR to keep" (oatmeal), NOT a blanket AWB toggle. Full detail: SESSION_NOTES Update 4 — Session 26.
...
LANDING PAGE — clozie.net — DONE 2026-07-27, live via Vercel on main:
- Hero CTA button (index.html line 142): href="#" → App Store URL
- Final CTA button (line 219): href="#" → App Store URL
- App Store URL: https://apps.apple.com/us/app/clozie-your-personal-stylist/id6775998682
- Instagram footer link (line 236): handle fixed styledbyclozie → styledby_clozie (underscore) in BOTH href and @text; confirmed from Grace's own IG profile
- Logo href="#" (line 129) left untouched by design
- 3 separate commits to main, each photo-verified before commit, all tap-tested on phone
- Repo: creatormama/clozie-website, main branch
