...
Last updated: **2026-08-04 — Update 4 — Session 29.** Fresh Mac harness reproduces the AWB washout to the digit (oatmeal brightGain 1.49, sC 1.00, R−B +44→+4, brightness 191→232). **"Read the room" scene classifier MEASURED and ruled out** — background cast cannot separate whites from pales (whitetee_warm bg +14 ≈ babyblue +15 but opposite treatment needed; oatmeal warm-garment-in-warm-room is indistinguishable; no true neutral reference in real rooms + capture-AWB already flattens the cast). "Turn AWB off" also rejected (regresses the whites). **Next = garment-only gentler AWB, chroma-threshold retune: protect chromatic pales (baby blue, peach, yellow) — no chromatic colour may go neutral; oatmeal→cream acceptable per Grace; whites stay white — tuned by Grace's eye on renders** (not started). Full detail in SESSION_NOTES Update 4 — Session 29. Standing constraints: **Build 29 / v1.0.5 is LIVE**; the Session 26 `MEASURE_COLOR_MODE` flag stays in code and MUST be removed before any App Store promotion; main `062d15b` / production `0baff39` untouched, cache 2,510; `AutoWhiteBalance.swift` unchanged (`f929d680…`).
