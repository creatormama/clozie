...
Last updated: **2026-08-06 — Update 4 — Session 31.** Build 31 / v1.0.6 (Candidate A camera-color fix ported to `AutoWhiteBalance.swift` commit `3a25976`; Session 26 `MEASURE_COLOR_MODE` scaffolding REMOVED commit `2a140d5`) built and TestFlight-delivered — **NOT promoted; Build 29 / v1.0.5 still LIVE on the App Store.** On-device: whites/pink/oatmeal/warm-yellow pass; **blue washes out (cool-side over-correction) = top next-session target.** Full detail: SESSION_NOTES Update 4 — Session 31. main `062d15b` / production `0baff39` untouched, cache 2,510.
