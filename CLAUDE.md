...
- Add Item / Edit Item X button inconsistent — the X close button on the Add Item and Edit Item panels ([App.js:1710](App.js:1710)) does not always close the panel reliably. Sometimes it closes correctly; other times the panel stays open. When it fails to close cleanly, the sticky "Set Today's Vibe →" bar at the bottom of My Closet disappears and only comes back on scroll or the next panel open. The Cancel button at the bottom of the panel ([App.js:1877](App.js:1877)) always works correctly and does not produce the bar-disappearing symptom. Surfaced 2026-05-17 during Session 10B testing. Working hypothesis (Session 10B read-only diagnosis): KeyboardAvoidingView layout race — when the keyboard is up at the moment X is tapped, KAV's bottom edge is still keyboard-adjusted when the sticky bar (position absolute, bottom 86 iOS / 70 Android) re-mounts via the `!showAddPanel` gate at App.js:1953, so the bar's computed screen position lands offscreen until a subsequent layout pass (scroll, next remount) corrects it. Floating + button at bottom 150/134 shares the same gate and may also be affected but hasn't been observed in this configuration. Two candidate fixes: cheap (call `Keyboard.dismiss()` in the X handler before `setShowAddPanel(false)`) or proper (move floating + and sticky bar OUTSIDE the KeyboardAvoidingView into a sibling absolute-positioned layer). Root cause needs investigation in a future session. Not blocking — bar reappears on scroll.
- Saved Outfits occasion chip filter returns 0 results for non-Casual-Day occasions even when `saved=true` rows exist with matching occasion strings. Source-code byte audit of `OCCASION_CHIPS` (App.js:2834) and `occasionOptions` (App.js:1992) both confirmed byte-clean UTF-8 middot `c2 b7` and byte-identical for every entry. "Casual Day" chip works (matches this-session generated saves where `lastPayload.occasion` was correctly written). All other chips return 0. Partial theory: `upsertOutfitInteraction` in `src/lib/outfitHistory.js` writes ALL snapshot fields on every UPSERT (including context fields like `occasion`), and Supabase upsert with these columns present overwrites existing values — so subsequent writes (unsave, rate, wear) done in a session without fresh `lastPayload` clobber the original `occasion` to NULL. Grace's dashboard inspection confirmed most saved rows have `occasion=NULL`. BUT — Grace also found a `Work · Office` row with `saved=true` and `occasion='Work · Office'` visually present in DB that still returns 0 results on the chip filter, which the context-clobbering theory doesn't explain. Surfaced 2026-05-17 (Session 12 S6 testing). Diagnostic plan: (1) run SQL `SELECT occasion, octet_length(occasion), encode(occasion::bytea, 'hex') FROM outfit_history WHERE saved = true` in Supabase to inspect the actual DB byte sequences; (2) if DB bytes match `576f726b20c2b7204f6666696365`, add temporary `console.log` to `filterSavedOutfits` to dump runtime bytes from `outfit.occasion` and `selectedOccasion` at compare time. S6-fix plan (read-before-upsert, omit context fields on UPDATE path) drafted but NOT applied — pending root-cause confirmation. Investigate Session 13. Full diagnostic plan + drafted fix in SESSION_13_BRIEF.md at project root.
...
**Bug surfaced mid-S6 testing — NOT fixed this session.** Search by text works correctly. "All" chip works (returns full list — filter no-ops). "Casual Day" chip filters correctly for outfits Grace generated AND saved in the current session. BUT all other occasion chips (Work · Office, Going Out, Formal Event, Outdoor · Sport, Weekend Errands, Travel) return 0 results even when Grace has saved outfits visibly tagged with those occasions in the Supabase dashboard. Diagnostic work performed in-session: (1) source-code byte audit of `OCCASION_CHIPS` (App.js:2834) confirmed clean UTF-8 middot `c2 b7`, 14 bytes for "Work · Office" specifically (hex `576f726b20c2b7204f6666696365`); (2) source-code byte audit of `occasionOptions` in TodaysVibeTab (App.js:1992) confirmed byte-identical to OCCASION_CHIPS; both source-of-truth arrays match for every entry, no mojibake, no zero-width characters, no non-breaking spaces. Theory developed: `upsertOutfitInteraction` in `src/lib/outfitHistory.js` calls `buildSnapshot(outfit, context)` on every UPSERT, which writes ALL snapshot fields including context fields (occasion, temperature, condition, brief, indoors, pinned_item_id), and Supabase upsert with these columns present in the row payload overwrites the existing DB values. So when a user does any subsequent interaction (unsave, rate, wear) in a session where `lastPayload === null` (e.g. reopened the app, didn't generate this session), all context fields get clobbered to NULL on the existing row — wiping the original occasion. Grace's dashboard inspection confirmed this for most rows: most saved rows show `occasion=NULL`, the only exceptions are this-session "Casual Day" generations where `lastPayload.occasion='Casual Day'` was set at save time. BUT — Grace also found one `Work · Office` row with `saved=true` AND `occasion='Work · Office'` visually present in the dashboard, that STILL returns 0 results on the Work · Office chip. So the context-clobbering theory explains most of the symptom but not this specific row. Diagnostic plan handed off to Session 13 (full plan in SESSION_13_BRIEF.md): (1) run SQL `SELECT client_outfit_id, occasion, octet_length(occasion), encode(occasion::bytea, 'hex') FROM outfit_history WHERE saved = true ORDER BY saved_at DESC` in Supabase to inspect actual DB byte sequences — if "Work · Office" row's hex differs from `576f726b20c2b7204f6666696365`, the bug is at the DB-write encoding layer; (2) if SQL bytes clean, add temporary `console.log` to `filterSavedOutfits` to dump runtime bytes from `outfit.occasion` and `selectedOccasion` at compare time. Either Diagnostic 1 or Diagnostic 2 will isolate the root cause; both are designed to be 1-minute checks. S6-fix plan drafted (read existing row before upsert via `.maybeSingle()`, omit context fields on UPDATE path, only include context fields on INSERT path so Supabase upsert preserves existing values on subsequent writes) but NOT applied this session — pending root-cause confirmation from Session 13 diagnostics. The S6-fix is good regardless and addresses the broader context-clobbering bug; whether an ADDITIONAL fix is needed depends on what the SQL hex shows.
...
- `src/lib/outfitHistory.js` — UNTOUCHED (S6-fix plan deferred to Session 13)
- `src/lib/outfitGeneration.js` — UNTOUCHED
...
- No SYSTEM_PROMPT changes — cache stays at 2,375 tokens
- No `src/lib/outfitHistory.js` changes — the S6-fix plan is drafted but pending Session 13 root-cause confirmation
- No `src/lib/outfitGeneration.js` changes
- No Supabase table schema changes — `outfit_history` table from Session 9A reused as-is
- No backfill of corrupted DB rows (rows with `occasion=NULL` from prior context-clobbering writes stay as-is; future writes will be correct once the S6-fix lands)
