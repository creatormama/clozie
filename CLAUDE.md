...
Last updated: **2026-07-13 — Update 4, Session 6 (Build 26, code — NO EAS build):** background-removal Swift module fully parameterized + JS wired on testing (8 per-step commits `da892f7`→`a2fc8bf`) — transparent PNG output, Core Image auto-enhance, baked silhouette shadow, and a Swift EXIF rider, ALL with NO-OP defaults so unwired output is byte-identical to Build 25; App.js `CUTOUT_OPTIONS` (png / enhance 1.0 / shadowOpacity 0.40 / blur 18 / offsetY 12 / gray 0.3) turns the new look ON at both call sites. On-device verification deferred to Build 26 Session 2 (TestFlight). Cache **2,510**, zero SYSTEM_PROMPT / Edge Function / eas.json / app.config.js / package.json change. Safety net UNCHANGED (`main` `062d15b` / `production` `f711c5d` / tag `v1.0.4-build25-appstore-live` → `f711c5d`). **Build 25 / v1.0.4 remains LIVE on the App Store; the 1.0.4 train is CLOSED and 1.0.5 is OPEN.** Full detail: SESSION_NOTES Update 4 — Session 6.
