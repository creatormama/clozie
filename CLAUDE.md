...
- Production pointer: branch `production` (tracking `origin/production`), currently at `f711c5d` (Build 25 / v1.0.4, live on the App Store).
...
Last updated: **2026-07-14 — Update 4, Session 8 (Build 27 — read-only cutout-quality audit):** READ-ONLY session, zero code changed, one file created (`BUILD27_QUALITY_FINDINGS.md`). Audited the Build 26 cutout complaints (whites read brown, hazy edges, over-diffuse shadow) against the actual Swift/JS pipeline. Root cause VERIFIED: the whites regression is the `enhanceStrength: 1.0` auto-enhance — Swift runs `autoAdjustmentFilters()` on the FULL warm frame BEFORE the mask (App.js:61 → Swift:118/123/126), the only op that recolors the garment; it is JS-tunable with no Swift recompile. Grace's decision — quality-first TWO-BUILD plan: **Build A (next native build, all Swift landed once):** `enhanceStrength → 0` (kills the whites regression), shadow `blur 18 → 12`, `resize 512 → 768` DECOUPLED from recognition (recognition stays 512 so the Anthropic payload doesn't grow), AND dormant garment-only white-balance + exposure + ~1px alpha-choke controls added at ZERO/no-op defaults so they ship inert and become JS-tunable afterward. **Build B (JS-only tuning, only if Build A falls short):** flip the dormant WB / exposure / choke values on — no Swift recompile. Build 26 (v1.0.5) stays TestFlight-only; Build 25 / v1.0.4 remains LIVE; 1.0.5 train OPEN. Full detail: SESSION_NOTES Update 4 — Session 8; findings in BUILD27_QUALITY_FINDINGS.md.
...
- Stale comment at `modules/expo-background-removal/src/BackgroundRemovalModule.ts:17` — reads "safe while Swift is still single-arg, Steps 1–7", but the Swift module became 2-arg in commit `e62b08a` (Build 26 Step 2). The 1-arg forwarding branch (`options === undefined`) is now dead in production (both `removeBackground` call sites pass `CUTOUT_OPTIONS`) and is `try/catch`-guarded to return `null` regardless, so it changes nothing at runtime. Pure comment cleanup, zero behavioral impact. Fold into the KNOWN ISSUES housekeeping migration, or fix in any future background-removal session. Surfaced 2026-07-14 (Update 4 — Session 7).
