...
Last updated: **2026-08-05 — Update 4 — Session 30.** Chroma-threshold retune **SOLVED on the Mac bench — Candidate A locked as winner by Grace's eye on all 10 locked photos** (2 whites, oatmeal, babyblue, 2 yellows, 2 pinks, 2 corals): whites clean, **every chromatic colour untouched, oatmeal stays TRUE oatmeal** (ideal tier, not the cream fallback). Parameters (full record in SESSION_NOTES Update 4 — Session 30): warm-side `sC = 1−smoothstep(0.06, 0.12, chroma)`; cool-side `sC = 1−smoothstep(0.18, 0.33, chroma) × smoothstep(1.35, 1.55, gainSpread)`; `sCMax 0.85`; all other AWB math verbatim shipped. Durable bench moved to **`~/Desktop/clozie-color-bench`** (10 locked photos, md5s in the notes; /tmp copy retired). Known limits: coloured garments render as captured (no cast-correction — ceiling of garment-only signals); unseen cool-side blues chroma 0.13–0.18 not proven safe; pink_warm bench-vs-device mismatch on record. **Build 30 Swift port = SEPARATE gated session — NO shipping code touched today**; `AutoWhiteBalance.swift` unchanged (`f929d680…`). Standing constraints: **Build 29 / v1.0.5 is LIVE**; the Session 26 `MEASURE_COLOR_MODE` flag stays in code and MUST be removed before any App Store promotion; main `062d15b` / production `0baff39` untouched, cache 2,510.
