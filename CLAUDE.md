...
APPLE DEVELOPER — Organization enrollment (Clozie LLC) SUBMITTED May 22, 2026. Waiting for Apple's response.
...
Updated May 23 2026 — Session 16B wired (Session Limits + VIP Bypass — Client Wiring). Five LOW-risk App.js substeps, each iPhone-tested before the next. (Step 1) Wardrobe cap 30 → 50: `maxItems` const + Free plan subscription card text both bumped. (Step 2) New `isVip` useState in MainAppScreen + new useEffect querying `vip_emails` table on mount + on SIGNED_IN / TOKEN_REFRESHED / USER_UPDATED. Uses `getSession()` (no network) for offline safety. Fail-safe to false on any error. Diagnostic `[VIP check]` log. (Step 3) Wardrobe cap enforcement + nudges + VIP bypass — `isVip` prop drilled to WardrobeTab, new derived `wardrobeNudge` (48 / 49 / 50 thresholds, null for VIPs), header count VIP-aware (`X items` vs `X/50 items`), new terracotta inline-text style entry, floating + button hidden at cap for non-VIPs, belt-and-suspenders early-return in `handleAddItem`. (Step 4) `handleGenerate` reads `response.sessionsUsedThisWeek` (new useState, reset on start, captured on success with null-safe guard) + `response.isVip` (sanity-check `[VIP mismatch]` warn vs client-side, no UI use). New `session_limit_reached` error-code branch maps to warm spec-quoted message. (Step 5) New derived `sessionNudge` (9 → "3 styling sessions left this week.", 11 → "1 styling session left this week.", null for VIPs); new conditional render between recovery banner and outfit cards map, new `sessionNudge` style. Edge Functions NOT touched, SYSTEM_PROMPT NOT touched, cache stays at 2,375 tokens, zero CLI deploys, zero new dependencies.
Updated May 23 2026 — Session 17 wired (security audit + dead-code investigation + Apple-review hidden-features verification — three read-only or reverted audits, ZERO code committed). (17A npm audit) Ran `npm audit` → 16 vulnerabilities (15 moderate + 1 HIGH `@xmldom/xmldom` XML injection inside Supabase realtime). Ran `npm audit fix` (NO --force, per pre-approval) — patched `@xmldom/xmldom` + `brace-expansion` + `ws` cleanly in `package-lock.json` only; `package.json` byte-identical. iPhone load surfaced "Cannot find module transform-worker.js" from `@expo/metro-config` — Metro bundler broken by `node_modules` repositioning even though only transitive deps moved. Reverted via `git checkout package-lock.json && npm install` — fully restored to 16-vuln pre-fix state, app boots clean. Postcss + uuid chain (the 13 remaining moderates) live deep inside `@expo/cli` / `@expo/config-plugins` / `expo-asset` / `expo-constants` / `expo-splash-screen` and only fix via `--force` which would silently upgrade Expo SDK 54 → 56 (BREAKING). All deferred to a coordinated future Expo SDK upgrade session. New KNOWN ISSUE locks the workflow: never run `npm audit fix` against Expo SDK 54. (17B dead-code cleanup) Investigated DEMO_MODE (6 references at App.js:2972-3411) + DEBUG_LAYOUTS test harness (8 references at App.js:3007-3754 — 8 fake layout fixtures, `buildDebugOutfit` / `openDebugModal` helpers, Mood Board layout switcher JSX, `isDebug` polaroid composition branch) — ~175 lines of dead code gated behind hardcoded `DEMO_MODE === false` and `id.startsWith('debug-')` checks that no production outfit ID matches. After the 17A npm scare and given imminent App Store submission, deferred all cleanup to a post-launch polish session per honest cost/benefit analysis: zero functional risk in production, zero Apple visibility, bundle-size delta negligible (~2-4 KB minified), and the harness itself is useful insurance for future visual-regression debugging. Zero changes to App.js. (17C hidden-features verification) Read-only audit of 8 Apple-review-critical hidden features. ALL CONFIRMED HIDDEN: (1) Complete The Look button at App.js:3581-3590 `{false && (...)}`; (1b) boutique panel at App.js:3592-3595 `{false && showBoutique[outfit.id] && (...)}` double-protected; (2) Store Suggestions in Mood Board at App.js:3761-3768 `{false && (...)}` wraps divider + label + "Boutique partners coming soon"; (3) Analyse My Wardrobe button at App.js:2022-2031 `{false && itemCount > 0 && !showAddPanel && (...)}` + Analyse message overlay at App.js:2033 `{false && showAnalyseMessage && (...)}` double-protected; (4) "What goes with this?" text at App.js:1794-1797 `{false && (...)}`; (5) Daily notifications PREFERENCES card at App.js:5866-5883 `{false && (...)}` wraps entire `<View style={settingsStyles.card}>` (full card hidden, not just toggle); (6) 94% match score at App.js:3489-3492 `{false && (...)}`; (7) "These N pieces create N×4 outfits together" fake math at App.js:3494-3497 `{false && (...)}`; (8) Google Sign-In button at App.js:619-634 `{false && (...)}` (Apple Sign-In at App.js:637 stays VISIBLE — separate concern outside this audit). Cross-checks: Subscription card `freeFeatures` array at App.js:5041-5051 lists only working features (5 hidden-feature names appear ONLY as inert JS line comments at App.js:5028-5040); `showBoutique` state at App.js:2993 + reset at App.js:3112 are harmless dead state (button hidden so state never flips). Zero changes to App.js. App is clean for Apple review on this front. Net zero code changes across Session 17 — 16 audit vulnerabilities unchanged (1 HIGH + 15 moderate), App.js byte-identical to start-of-session, all Edge Functions untouched, SYSTEM_PROMPT untouched, cache stays at 2,375 tokens, zero CLI deploys, zero new dependencies.
Updated May 23 2026 — Mood Board shoes resizeMode fix wired. One-line surgical edit in App.js MoodPolaroid component ([App.js:2770](App.js:2770)): `<Image>` gained `resizeMode={item?.category === 'Shoes' ? 'contain' : 'cover'}` prop. Shoes (typically landscape) now render as `contain` inside the portrait polaroid frame — full toe-to-heel visible with small letterbox top/bottom. All other categories (Tops, Bottoms, Dresses, Outerwear, Accessories) keep default `cover` behavior — render byte-identical to pre-fix. MoodAccCell accessory grid cells deliberately untouched (cells at 30-40px would render shoes as tiny floating dots under `contain`; re-evaluate only if a real shoe ever lands in an accessory grid). Closes the shoes-crop discomfort surfaced after the Session 13C `resizeMode="contain"` blanket-apply attempt was reverted on Grace's call (contain across all categories made tops/dresses look small and floating). Category-aware approach addresses the actual problem (landscape shoes side-cropping in portrait frames) without sacrificing the polaroid look for garments. App.js net diff +5 / -1 lines. Edge Function NOT touched, SYSTEM_PROMPT NOT touched, cache stays at 2,375 tokens, zero CLI deploys, zero new dependencies. iPhone-verified across shoe outfits + tops/dresses for regression.
Updated May 23 2026 — Session 17F wired (SYSTEM_PROMPT Rule 13 + 3-check server-side outfit validation). Three CLI deploys to generate-outfits Edge Function. Deploy 1: Rule 13 added between Rule 12 and VOICE section ("Every outfit MUST include at least one Top or one Dress. ... Never output two Bottoms in one outfit. Never output an outfit built from only Accessories, Bottoms, and Shoes — the core upper piece must always be present."). UTF-8 byte-verified — em-dash count 33 → 34, zero middots, zero mojibake. SYSTEM_PROMPT cache 2,375 → 2,442 tokens (~19% headroom above 2,048 threshold). Round-trip verified via Supabase Logs. Deploy 2: server-side Check 1 (Top/Dress requirement → positional `buildSmartFallback` replacement, distinct outfits preserved) and Check 2 (Accessories trim if outfit > 6 items) inside `if (mapped)` block — Map<id, Item> lookup, dynamic `finalSource` ('sonnet' unless all 3 replaced), graceful try/catch on `buildSmartFallback`. Deploy 3: Check 3 (Bottoms dedupe with pinned-preference — `keepId = (pinned && bottomIds.includes(pinned.id)) ? pinned.id : bottomIds[0]` preserves pinned-item contract from `validateAndMapOutfits` line 714 if user pins a Bottom). Cache stayed at 2,442 across all post-Deploy-2 and post-Deploy-3 calls. End-to-end iPhone verified: all generates `source: 'sonnet'`, zero `structural fix:` log lines (Rule 13 alone sufficient on tested wardrobes), zero errors, outfits structurally normal across Outdoor · Sport, Casual Day, Going Out, Work · Office. Closes the May 19 (Session 13C) Known Issue about Sonnet two-bottoms-no-top outfits AND the residual "Top + 2 Bottoms" edge case in one session. App.js NOT touched, recognize-photo/delete-user NOT touched, no Supabase table changes, no new dependencies. Session 13E client-side Hanger View outerwear-promotion fallback kept in place as third-line defense.
