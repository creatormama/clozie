...
**UPDATE 4 LIVE: Build 29 / v1.0.5 — Apple-APPROVED and RELEASED on the App Store 2026-07-21 (Grace pressed Release in App Store Connect; US, 1 region; rollout in progress). Build 29 / v1.0.5 was the LIVE App Store build until Build 32 / v1.0.6 replaced it (2026-08-10).** (Builds 26/27/28 were TestFlight-only and never released; Build 25 / v1.0.4 was the prior live build.) Ships the phase15D automatic white balance (AWB) white-fix — `modules/expo-background-removal/ios/AutoWhiteBalance.swift`, app-code commit `5b51910` (Session 19). EAS Build 29 was built from commit `0baff39` — VERIFIED via `eas build:view` server record (Version 1.0.5 / Build 29 / Commit `0baff3924ec…`). **The v1.0.5 train is now CLOSED — per the VERSION RULE, any future app CODE change requires bumping the version to 1.0.6 in BOTH `app.config.js` AND `package.json` FIRST, or Apple rejects the build with 90062/90186.** (Edge Function deploys are server-side and NOT gated by this — they ship independently of the app version.) **Release git bookkeeping DONE 2026-07-21:** annotated tag `v1.0.5-build29-appstore-live` (tag-object `e2b79f2`) created on the shipped Build 29 commit `0baff39` AND `production` fast-forwarded `f711c5d` → `0baff39` (true fast-forward, 41 commits, nothing lost); both pushed to origin. `production` now points at the live Build 29. The Build 25 restore tag `v1.0.4-build25-appstore-live` (tag-object `d219721`) @ `f711c5d` stays permanent; the descriptive restore tag `v1.0.5-build29-awb-whitefix` @ `0baff39` also remains.

**UPDATE 4 LIVE: Build 32 / v1.0.6 — Apple-APPROVED and RELEASED on the App Store 2026-08-10 (submitted 2026-08-09; Grace pressed "Release This Version" ~3:20 PM EDT — manual release, all users at once, age rating kept). Build 32 / v1.0.6 is now the LIVE App Store build.** (Build 30 / Build 31 were TestFlight-only and never released; Build 29 / v1.0.5 was the prior live build.) Ships the recognition-veto (Variant i, cool-side-only) camera-colour fix — App.js commit `d34b05c`, native AWB `min(sC, coolCap)` hook commit `f649506` (Session 35). EAS Build 32 built from commit `d34b05c`, profile preview — VERIFIED via `eas build:list` server record (Version 1.0.6 / Build 32 / Commit `d34b05c3798…`). **The v1.0.6 train is now CLOSED — per the VERSION RULE, any future app CODE change requires bumping the version to 1.0.7 in BOTH `app.config.js` AND `package.json` FIRST, or Apple rejects the build with 90062/90186.** (Edge Function deploys are server-side and NOT gated by this.) **Release git bookkeeping DONE 2026-08-11:** annotated tag `v1.0.6-build32-appstore-live` (tag-object `b8424fc`) created on the shipped Build 32 commit `d34b05c` AND `production` fast-forwarded `0baff39` → `d34b05c` (true fast-forward, no force); both pushed to origin. `production` now points at the live Build 32. The Build 29 restore tag `v1.0.5-build29-appstore-live` (tag-object `e2b79f2`) @ `0baff39` stays permanent; the TestFlight tag `v1.0.6-build31-testflight` @ `2a140d5` also remains.
...
- Production pointer: branch `production` (tracking `origin/production`), currently at `d34b05c` (Build 32 / v1.0.6, live on the App Store; fast-forwarded from `0baff39` on 2026-08-11).
...
Last updated: **2026-08-14 — Update 4 — Session 37.** Blank-photo fix (closet + Your Looks) SHIPPED to TestFlight as **Build 33 / v1.0.7** (EAS profile preview, commit `9b6e8f7`, finished first try, server-verified; delivered via Transporter, no `eas submit`). Root cause: Supabase signed photo URLs expire at 1h and were re-minted only on mount, and today's `generatedOutfits` never re-hydrated. Fix = 5 edits on `9b6e8f7` (version 1.0.6→1.0.7; preserve itemIds at the generation resolve; new generatedOutfits re-hydration effect keyed on [wardrobeItems]; dedicated foreground signed-URL re-mint with 45-min guard + catch-and-keep; TTL 3600→86400). **Tested overnight on device — HOLDING, photos no longer blank.** App Store release DEFERRED until Grace returns from vacation — **App Store still on Build 32 / v1.0.6; production `d34b05c` UNMOVED; no tags.** testing → `9b6e8f7` (+ this docs commit); main `062d15b` untouched; zero Edge Function deploys; cache 2,510. Full detail: SESSION_NOTES Update 4 — Session 37.

**2026-08-11 — Update 4 — Session 36.** Build 32 / v1.0.6 RELEASED to the App Store — now the LIVE build. Submitted 2026-08-09, Apple-approved 2026-08-10, Grace pressed Release ~3:20 PM EDT (manual, all users at once, age rating kept), superseding Build 29 / v1.0.5. Release git bookkeeping DONE + verified on origin: annotated tag `v1.0.6-build32-appstore-live` (tag-object `b8424fc`) on shipped commit `d34b05c` AND `production` fast-forwarded `0baff39` → `d34b05c` (true fast-forward, no force); both pushed. v1.0.6 train now CLOSED (next app CODE change bumps BOTH app.config.js + package.json to 1.0.7 first). testing `5d117ef` / main `062d15b` untouched; zero code changed; zero Edge Function deploys; cache 2,510. Full detail: SESSION_NOTES Update 4 — Session 36.

**2026-08-09 — Update 4 — Session 35.** Recognition veto (Variant i) PORTED + BUILT: **Build 32 / v1.0.6** (EAS profile preview, commit `d34b05c`, FINISHED first try, server-verified) TestFlight-delivered via Transporter — **first-pass device test POSITIVE** (blues stay blue in both observed lights, whites clean, oatmeal unchanged, airplane-mode 0.4-cap path verified with offline bar + cutout + colour kept, camera AND library OK, delayed cutout acceptable) — **extended testing continuing; Build 32 stays TestFlight-only, NOT promoted; Build 29 / v1.0.5 still LIVE.** Locked decisions: (a) offline/timeout fallback cap 0.4; (b) Option 1 sequence — recognition first, colour answer raced vs 8000 ms, recognition never cancelled; (c) word list neutral={white, cream} + filler={off, optic}, tokenize on non-letters, unknown = veto — **ivory provisionally on, then REVERSED OUT by Grace** (bench swatch showed voucher-ivory→white; list exists only for words whites actually need). Port = 4 files +70/−7 (App.js sequence + helper; AutoWhiteBalance.swift `min(sC, coolCap)` cool-branch hook, Candidate A constants verbatim; module `awbCoolCap` field; TS type) — commits `f649506` + `d34b05c` on testing, pushed. Bench gates passed in `~/Desktop/clozie-color-bench/session35_port/` BEFORE any edit (determinism md5, 10-photo numeric parity, variant-(i) proof, gray/ivory/ecru swatches; md5-composite parity honestly substituted with exact numeric parity). **Build 31 restore tag `v1.0.6-build31-testflight` @ `2a140d5` created + pushed** (not -appstore-live; production unmoved). Profile catch on record: `--profile production` proposed, caught by Grace, corrected to the locked `--profile preview` (eas.json + EAS server history: Builds 29/30/31 all preview). Old `Promise.all` fork kept as comments for revert until extended device testing passes. Full detail: SESSION_NOTES Update 4 — Session 35. main `062d15b` / production `0baff39` untouched, cache 2,510.

**2026-08-08 — Update 4 — Session 34.** Fix direction chosen + paper-tested (READ-ONLY + bench; 62 production recognize-photo calls, zero deploys): recognition veto (Direction 1) HOLDS — blues named blue 15/15, whites vouchered 9/9, zero dangerous mistakes; ensemble direction killed by measurement; blue catastrophe proven 100% cool-side (5 harms / 1 good across 62). Grace locked Variant (i) cool-side-only veto (all 10 locked photos byte-identical; eye-passed) + purely-neutral-names mapping rule + Direction 2 cool cap as offline fallback; variant (ii) preserved in session34_fix/ as the designated future tune-up. NO Swift, NO prototype; Candidate A LOCKED; shipping code untouched. The Swift/App.js port of variant (i) is a SEPARATE future gated session. Build 31 stays TestFlight-only; Build 29 / v1.0.5 still LIVE. Full detail: SESSION_NOTES Update 4 — Session 34. main `062d15b` / production `0baff39` untouched, cache 2,510.

**2026-08-08 — Update 4 — Session 33.** Scatter measurement — Hypothesis 1 CONFIRMED and extended: spread, chroma, AND warm/cool side-read all scatter capture-to-capture (blue cardigan spans the full 1.35–1.55 gate on one sweater; 512-wide device-recipe run canonical per App.js:1806/1838). Candidate A LOCKED and untouched; shipping code untouched; no fixes designed. Build 31 stays TestFlight-only; Build 29 / v1.0.5 still LIVE. Full detail: SESSION_NOTES Update 4 — Session 33. main `062d15b` / production `0baff39` untouched, cache 2,510.
