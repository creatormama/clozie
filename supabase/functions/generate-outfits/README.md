...
Updated: 2026-05-14 (Session 7b-7 — dislikes hard filter added inside `applySafetyFilters` after the existing C1–C5 / occasion filters. Reads `styleProfile.neverWear` from request body, tokenizes (split on commas/semicolons, lowercase, trim, drop stopwords, minimum length 4), matches case-insensitive substring on `name + colour` only (not notes — too noisy), pinned item exempt. Soft-fail safety net unchanged. Regenerate button in App.js wired to re-fire `handleGenerate(lastPayload)` (was a fake 2-second `setTimeout` from Session 7b-2). SYSTEM_PROMPT untouched — cache safe at ~2,375 tokens post-7b-6 cleanup. Known curiosity: the `dislikes filter dropped` log line does not appear in Supabase Logs even when the filter is firing (iPhone behavior confirms drops happen); separate investigation for a future polish pass).
Updated: 2026-05-14 (Session 7C — JavaScript Smart Fallback: when Anthropic fails for any reason (timeout / 5xx / 429 / malformed JSON / schema validation / name→UUID mapping failure), `buildSmartFallback` now fires instead of the basic stub. Returns 3 outfits with editorial-occasion names (per-occasion pools: Casual Day → "Easy Sunday" etc., Work · Office → "Morning Confidence" etc., Going Out → "Night Mode" etc., Formal Event, Outdoor · Sport, Weekend Errands, Travel), color-aware composition (neutral / earth / navy family pairing with navy+earth clash detection), pinned-item enforcement, and item-aware descriptions ("[colour first-word] with [colour first-word] — [mood]."). Uses `filteredItems` (safety-filtered pool), with soft-fail revert to unfiltered `items` if pool went thin. If `buildSmartFallback` itself throws, last-resort to existing `buildStubOutfits` — 3-tier safety net (Sonnet → smart fallback → stub) live in production. New `source` field values: `"sonnet" | "fallback" | "stub"`. SYSTEM_PROMPT untouched — cache safe at 2,375 across all session calls. App.js NOT touched.).
...
5. **Safety filters** (`applySafetyFilters`) — reshape the wardrobe pool before Sonnet sees it. Pinned item always exempt. If filters drop the pool below the essentials check, the soft-fail net reverts to the unfiltered pool (gate 4 already proved it viable).
   ...
   - **Dislikes** (Session 7b-7) — drop items whose `name` or `colour` contains any token from the user's "I never want to wear" text. Tokenized: split on `,` and `;`, lowercase, trim, drop empties, drop stopwords (`anything`, `the`, `a`, `an`, `no`, `hate`, `nothing`, `with`), minimum token length 4 (avoids `tan` matching `tank top`, `red` matching `adidas`). Match on `name + colour` only — `notes` is free-form text and would over-filter. Pinned item exempt (user pinned it deliberately — overrides dislikes). Soft-fail safety net catches over-filtering scenarios.
...
8. **Silent 3-tier fallback** on any AI/validation failure (network error, timeout, non-2xx, JSON parse failure, schema validation failure, name → UUID mapping failure, missing pinned item):
   - **Tier 1 — Smart fallback (Session 7C):** `buildSmartFallback` fires using the safety-filtered pool (`filteredItems`). Color-aware composition prefers compatible neutral/earth/navy family combinations. Names picked at random (no repeats) from a per-occasion editorial pool. Vibes drawn from a per-occasion vibe pool. Descriptions reference actual item colors and fabric words. Pinned item appears in every outfit. If the filtered pool went thin (< 5 items), reverts to unfiltered `items` — gate 4 already proved them viable.
   - **Tier 2 — Stub composition (Session 7b-1):** kept as last-resort net. Fires only if `buildSmartFallback` itself throws a runtime exception. Uses unfiltered `items`. Hardcoded names ("Morning Coffee Run", "Studio to Street", "Quiet Confidence"). Random pick from category.
   - The user always gets 3 outfits.
9. Returns `{ outfits: [...3...], source: "sonnet" | "fallback" | "stub" }`. Each outfit has `{ id, vibe, name, description, items: [item_id, ...], styleMatchScore }`. The `source` field is a debug marker — `"sonnet"` for real AI, `"fallback"` for the smart fallback (Session 7C), `"stub"` for the last-resort stub (Session 7b-1).
