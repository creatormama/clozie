...
**Edge Function deploys:** 0. **Cache:** 2,510 (untouched). **EAS builds:** 1 — **Build 32 / v1.0.6, profile preview, commit `d34b05c`, FINISHED first try** (server-verified via `eas build:view`: Build 32 | v1.0.6 | preview | d34b05c3 | artifact ready). Delivered via Transporter by Grace; NO `eas submit`; TestFlight internal only.

**DECISIONS LOCKED (Grace, 2026-08-09):** **(a)** offline/timeout/error fallback = cool-side cap **0.4**. **(b) Option 1 sequence** — recognition first; ONLY its colour answer raced vs **8000 ms** timeout (revisit on device); timeout/offline/error resolves to the 0.4 cap; recognition NEVER cancelled (a late answer still fills the fields). Code basis from the Step 3 investigation: the Add button already waits on recognition (`isScanning` gate), so sequencing costs ~1-2 s of Add-wait; the visible cost is the cutout appearing after the colour answer. **(c) word list — FINAL after a mid-session reversal: neutral = {white, cream} · filler = {off, optic} · tokenize lowercase on non-letters · voucher ONLY if every token ∈ neutral ∪ filler · ANY other token = veto (unknown-fails-to-veto is the safety spine). IVORY was provisionally ON, then REVERSED OUT by Grace** after the bench swatch showed voucher-ivory→white under a strong cool cast ((159,185,203)→(239,244,248)); her locked principle: the list exists only for words whites actually need (observed white vouchers across all 62 production calls: Cream ×9 + Optic White ×1; standalone "Ivory" 0/62). Gray was dropped earlier on math: brightGain is anchored to the garment's own neutral luma (clamp 3.0) so a mid-gray CAN be lifted ~128→~200 — "gray cannot become white" is FALSE in this pipeline. Discovery on record: the recognize-photo prompt itself steers the vocabulary ("Cream not White", "Charcoal not Dark Grey" — README.md:52), so the naming habit is largely under our control; prompt wording left UNTOUCHED this session (post-verification question logged).

**Chunk 1 bench gate (`session35_port/`, Grace's eye passed the contact sheet BEFORE any shipping edit):** `render_port.swift` = Session 34's renderer + the exact planned hook `sC = min(sC, COOL_CAP)` (cool branch only). Gates: (1) determinism — Session 34's own binary re-run produced a byte-identical file (md5 `c1ecf269…`); (2) numeric parity — ALL 10 locked photos reproduce Session 34's RAW + Candidate A body-means exactly (whitetee_daylight chroma 0.1308, babyblue 0.0882 / spread 1.358 / sC 0.005); (3) variant-(i) proof — oatmeal at cap 0 KEEPS its warm-side 0.191 correction (cool-only veto cannot touch warm side). Swatches gray/ivory/ecru × warm/cool/coolstrong casts: gray cool-voucher washes (85,102,120)→(156,159,164) — veto column protects it; moderate cool cast landed ivory at spread 1.344 vs gate floor 1.35 (knife-edge reproduced synthetically). **Honest substitution on record:** the plan promised md5 parity against the Session 30/34 composite PNGs — unattainable across tools (different panel counts/titles change bytes); substituted: md5 determinism of the old tool itself + exact integer body-mean + 4-decimal sC parity on every photo.

**Port (Chunks 2-9, each Grace-YES'd; 4 files, +70/−7):** AutoWhiteBalance.swift — `corrected(_:coolCap: Float = 1.0)` + `min(sC, coolCap)` in the cool branch only, Candidate A constants verbatim (commit `f649506`). BackgroundRemovalModule.swift — `@Field var awbCoolCap: Double = 1.0` + pass-through `Float(options?.awbCoolCap ?? 1.0)` (same commit). BackgroundRemoval.types.ts — `awbCoolCap?: number` declaration (same commit). App.js (commit `d34b05c`) — dormant helper `coolAwbCapForColor` + constants (node-tested by extracting the exact shipped lines: all 28 observed COLOUR strings → vouchers exactly {Cream, Optic White}; the 7 decision-(c) test words + Ivory/Grey/Gray all veto; null/undefined/empty → 0.4); `runRecognition` returns the colour word (null on failure; can never reject — field-filling/retake logic byte-identical); `applyBackgroundRemoval(uri, coolCap = 1.0)` → `{ ...CUTOUT_OPTIONS, awbCoolCap }`; BOTH photo handlers sequenced with byte-identical blocks (anchor-extracted and diffed; old `Promise.all` kept as a comment for instant revert until device-verified); hidden test screen passes explicit `AWB_OFFLINE_COOL_CAP` (no recognition on that path). Known cosmetic edge on record: an all-filler colour name ("Off" alone) would voucher under the literal locked rule — judged imaginary (0/62, prompt demands fashion names), left as locked.

**PROFILE CATCH (Grace caught it, pre-build):** the proposed build command said `--profile production`; the locked convention is `--profile preview`. Evidence gathered before building: eas.json defines preview {distribution: "store", ios autoIncrement} and production {ios autoIncrement}; the EAS server build list shows Builds 29/30/31 ALL used profile **preview** (and independently re-confirmed Build 31's commit = `2a140d50`). Built with preview.

**Tests:** bench gates above + **first-pass ON-DEVICE (Build 32 via TestFlight): POSITIVE** — blues stay blue (both observed lights), whites clean, oatmeal unchanged, airplane-mode 0.4-cap path verified (offline bar + cutout appears + colour kept), camera AND library paths OK, delayed-cutout trade acceptable to Grace. **Extended testing CONTINUING. Build 32 stays TestFlight-only — NOT promoted; Build 29 / v1.0.5 still LIVE on the App Store.**

**UNVERIFIED / OPEN:** extended device matrix (rapid retake mid-scan, more garments/lights, a real gray/silver garment); the 8000 ms timeout value (device revisit); Hypothesis 2 (Build 31 brightness anomaly / port numeric parity) still OPEN; variant (ii) full veto preserved in session34_fix/ as the designated future tune-up; recognize-photo prompt-wording question ("Cream not White" steering) logged for post-verification; App Store promotion of the veto = separate future decision. **Files (`session35_port/`):** render_port.swift + bin, 19 `port_*.png` (10 locked + 9 swatches), `port_contact_sheet.jpg`, `swatch_output.txt`, `veto_babyblue.png` (determinism re-run), scratch helper test in the session scratchpad.

---

## Update 4 — Session 34 — 2026-08-08 — FIX DIRECTION CHOSEN + PAPER-TESTED: recognition veto (Direction 1) HOLDS on real recognition — 62 production recognize-photo calls (READ-ONLY use, zero deploys): blues named blue 15/15, whites vouchered 9/9, zero dangerous mistakes. Grace locked Variant (i) cool-side-only veto (all 10 locked photos byte-identical, eye-passed) + purely-neutral mapping rule + Direction 2 cool-cap as offline fallback. NO Swift, NO prototype; Candidate A and shipping code untouched.

**Branch:** testing @ `9fbd194` (Session 33 close-out) unchanged all session — this entry + a CLAUDE.md pointer are the only commits. main `062d15b` / production `0baff39` UNTOUCHED. Build 29 / v1.0.5 still LIVE; Build 31 stays TestFlight-only. Step 1 safety check: testing 0/0 with origin (`git ls-remote`, no fetch); all 10 locked photos md5-PASS vs the Session 30 record; session33_scatter canonical files + prepped512 (52) + session32_diag present; Candidate A constants verbatim in AutoWhiteBalance.swift (0.06/0.12 · 0.18/0.33 · smoothstep(1.35,1.55) · sCMax 0.85 · plausDamp 1.0).

**Edge Function deploys:** 0 (62 READ-ONLY invocations of the deployed recognize-photo — calls only, ~$0.30-0.50 API spend, Grace-approved ~$1-2 budget). **Cache:** 2,510 (untouched). **EAS builds:** 0. **Session type:** read-only repo + bench analysis; all outputs in NEW scratch `~/Desktop/clozie-color-bench/session34_fix/`; locked photos / session32_diag / session33_scatter read-only inputs.

**Step 2 — fix-direction analysis:** (1) Ensemble/self-confidence direction KILLED by measurement — 12 captures × 5 trivial perturbations (flip/crop/resizes) through the verbatim shipped estimator: max sC wobble 0.046; the 1875 full-wash is confidently, stably wrong at 0.000 wobble. The Session 33 scatter is BETWEEN captures (camera varies shot-to-shot), not within one — no internal confidence signal exists. (2) Cool-side harm/benefit tally across all 62 inputs: cool-side corrections = 5 harms (ALL blue garments) / 1 good (locked whitetee_daylight) — the blue catastrophe is 100% cool-side; every Build-31 floor color lives on the warm side. (3) Raw garment hue NOT capture-stable (blue cardigan R−B −37..+15, sign flips under warm light) — kills closet-color-matching; closet-consistency also near-zero coverage (one capture per garment in real use). Three directions presented; Grace picked Direction 1 (recognition veto — Clozie's own recognition already names colors from the pre-AWB fixed.uri, App.js:1811 Promise.all) with Direction 2 (cool-side cap) as fallback, PENDING the paper test.

**Step 3 — Stage 2 paper test (Grace-approved, incl. auth approach):** 62 calls to production recognize-photo on 10 locked + 52 canonical prepped512; 0 errors, 0 retries. Auth: one-time admin-minted session for Grace's own account via the Keychain PAT (no password handled, no account created, ~1h expiry). RESULTS — Q1: blue cardigan 8/8 named blue (Baby/Sky/Powder Blue) INCLUDING both warm-light captures whose pixels read warm. Q2: denim 7/7 (Light Wash Blue/Denim). Q3: whites 9/9 vouchered (Optic White / Cream) — whitetee_daylight (blue-cast photo, the mirror-image risk) named "Cream", keeps its (218,215,216) clean. Q4: ZERO real colors named White; one near-miss (sage 1890 → "Cream") vouchers = today's Candidate A behavior exactly. Structural property confirmed on all 62: the veto only REMOVES correction — worst case anywhere = Build 31 behavior; it can never create a new harm. Mapping flaw found: substring rule vouchers compound names ("Sage Grey", "Coral & Cream Polka Dot") — cosmetic on this dataset (all such sC=0), structural for the future. 23 RAW | Candidate A | A+veto renders + contact sheet produced (render_veto = verbatim-pipeline bench tool).

**DECISIONS (Grace, 2026-08-08, locked):** (1) **Variant (i) — cool-side-only veto — WINS and ships first** (blues beautiful, whites white, colors hold; all 10 locked photos byte-identical under (i)). (2) **Mapping rule refined + locked:** veto if ANY real color word appears; voucher only purely-neutral names. (3) **Direction 2 cool-side cap = offline/recognition-unavailable fallback** (cap value = parameter decision for the port session). **Variant (ii) full veto PRESERVED in session34_fix/** (numbers, tables, renders) as the designated future tune-up (sage/mauve warm-side wobble), bench-proven, to be considered ONLY after variant (i) is device-verified and passes Grace's eye on the phone.

**Tests:** bench-only; no build, no device tests. **UNVERIFIED:** variant (i) exists only as bench simulation + renders — NOT ported, NOT built, NOT device-tested; the Swift/App.js port is a SEPARATE future gated session (App.js ordering recognition→AWB or rare re-run, color-word→family map, native-module hint param, fallback wiring). Naming stability measured on these 62 captures (one wardrobe, two lights) — broader-wardrobe naming unverified. Hypothesis 2 (Build 31 brightness anomaly / port parity) untouched, stays OPEN. **Files (session34_fix/):** FINDINGS_step2.md + FINDINGS_step3.md (incl. locked decisions) · perturb/ (60) + perturb_output.txt + perturb_table.txt + parse_perturb.py · run_recognition.sh + call_recognition.py + results.jsonl · naming_table.txt · render_veto.swift/bin + render_veto_output.txt · 23 veto_*.png + veto_contact_sheet.jpg. cache 2,510; Build 29 / v1.0.5 LIVE.
...
**Commits (this session, testing, named-file only — no `git add -A`, no amend, no push):** (1) this SESSION_NOTES.md entry; (2) CLAUDE.md CURRENT BUILD STATE pointer. Both "This commits to testing, not main."
**Edge Function deploys:** 0. **Cache token count:** 2,510 (SYSTEM_PROMPT untouched). **EAS builds:** 0. **Session type:** repo READ-ONLY + autonomous bench tuning. `AutoWhiteBalance.swift` READ only, never edited (still md5 `f929d680…`). `App.js` READ only (export-code verification).

**DURABLE BENCH — new standing location: `~/Desktop/clozie-color-bench`** (Grace's call: /tmp is wiped on reboot). Full copy of the Session 29 scratchpad, md5-verified, harness re-run from the new location reproduces every Session 29 digit (oatmeal brightGain 1.4932, sC 1.0000, corrected 234,232,230). The /tmp copy is retired untouched. Caveat on record: the old `render_baseline` binary hardcodes the /tmp photos path — the new tools (`measure_any`, `sweep`, `render_cand`) use the durable paths; use those going forward.

**10-PHOTO LOCKED SET (photos/, md5):** whitetee_warm `9e313ed70cbb465116655310721956fb` · whitetee_daylight `5c09898ab5b182894d6fe7a1b74cee7b` · oatmeal `2b87af4bd0d50f3ada7e1549580417cf` · babyblue `f3187d65d6a0a3d3450e4662b42b41d0` · yellow_warm `462ec7877ce801fbb67d43d7922ee485` · yellow_daylight `e0d5d0e1c93fcacb03356cdd4bcd712c` · pink_warm `cec533d97b127fdac43735e252f39461` · pink_daylight `7c36f4f4108da69dde35a523786cbed3` · coral_warm `7caead651eccc01bd45931c2cb82d603` · coral_daylight `77c1b4b663cab066e61aaa0c63029ccc`. All 6 new captures VERIFIED equivalent inputs: exported at `fixed.uri` (App.js:1829 camera / App.js:1862 library) = post-ImageManipulator resize-512/JPEG-0.75, PRE-recognition, PRE-background-removal/AWB; all 512×683 Display P3, no alpha, no camera EXIF — same artifact class as the Session 29 four. Warm-vs-daylight and pink-vs-coral identified from pixels AND by eye (red-rug room = warm light; beige-couch spot = daylight); numbers-only hue splitting was demonstrably wrong on the coral pair — eyeball check is mandatory for future capture IDs.

**KEY MEASUREMENT — plain global protLo retune is mathematically dead:** provisional meanBodyChroma interleaves fatally — must-protect babyblue **0.0882** sits BELOW must-correct whitetee_daylight **0.1308**, and pink_daylight **0.1294** / yellow_daylight **0.1402** / pink_warm **0.1469** straddle it. No single global threshold can pass the gate hierarchy. Proven before any render.

**WINNER — CANDIDATE A, LOCKED by Grace's eyeball on all 10 renders (2026-08-05). Full parameter record (all other AWB math verbatim shipped):**
- **Warm side** (illumR ≥ illumB): `sC = 1 − smoothstep(protLoW 0.06, protHiW 0.12, meanBodyChroma)` (shipped: 0.22/0.44 both sides).
- **Cool side:** `sC = 1 − smoothstep(protLoC 0.18, protHiC 0.33, meanBodyChroma)` **× smoothstep(1.35, 1.55, gainSpread)** where `gainSpread = max(gainR,gainB)/min(gainR,gainB)` — the new garment-only signal: the estimator demands a much stronger fix for a true daylight cast on a white (spread 1.80) than for babyblue's own colour (1.36).
- **sCMax 0.85** ceiling on correction strength. brightCap and plausDamp dials evaluated and left OFF (inert on this set).
- Plain English: AWB now fires only on genuinely near-neutral garments; anything with its own colour is left as captured; cool-side correction additionally requires the estimator to clearly diagnose a lighting problem.

**Results (raw → Candidate A, body mean sRGB / R−B):** whitetee_warm (237,223,207)+30 → (244,243,242)+2 CLEAN · whitetee_daylight (164,177,203)−39 → (218,215,216)+2 CLEAN (more neutral than shipped's +11) · oatmeal (212,193,168)+44 → (218,202,181)+37 TRUE OATMEAL · babyblue −30 → −30 UNTOUCHED · yellow_warm +100 → +100 · yellow_daylight +50 → +50 · pink_warm +116 → +116 · pink_daylight +49 → +49 · coral_warm +122 → +123 · coral_daylight +73 → +73. Sweep: 3,456 candidates × 10 photos, 1,044 passed all hard gates; harness sanity reproduced every shipped-baseline digit at full res. **RUNNER-UP/FALLBACK — CANDIDATE B, documented in full:** identical to A in every parameter except **sCMax = 1.00** (A uses 0.85) — i.e. warm-side 0.06/0.12, cool-side 0.18/0.33 × spread-gate smoothstep(1.35, 1.55), whites corrected at FULL strength: whitetee_warm → (245,246,247) R−B −2, whitetee_daylight → (226,220,215) R−B +11; all 8 coloured photos byte-identical to A. Grace judged A-vs-B no meaningful difference by eye — A chosen; **switching to B later is the one-line change `sCMax 0.85 → 1.00`, no re-derivation needed.** Candidate C (oatmeal→cream) REJECTED.

**ON RECORD — pink_warm bench-vs-device mismatch:** shipped-AWB bench math washes pink_warm to (250,248,234) R−B +16, but Grace's field report says the warm pink capture KEPT pink on the on-device closet card (only the daylight pink washed, labeled "Blush Peach"). 1 of 10 field observations disagrees with the bench; the other 9 match. Unresolved; moot under Candidate A (pink_warm untouched either way), but must be remembered if the device port behaves unexpectedly.

**Honest limits on record:** (1) Candidate A never colour-corrects a cast on a coloured garment — daylight yellow/pink render as captured (slightly muted); that is the ceiling of garment-only signals (Session 29 proved the room can't help). (2) Unseen-blues risk: a blue garment with cool-side chroma 0.13–0.18 whose self-estimate has spread >1.55 would still be corrected toward gray — no such garment in the test set; zone not proven empty. (3) Whites with chroma just above 0.06 get partial (ramped) correction — behaviour degrades gracefully, but a creamy white will stay slightly creamy.

**Tests:** none on device — bench only. **UNVERIFIED:** Candidate A exists only in the bench (`sweep.swift` / `render_cand.swift`); NOT ported to `AutoWhiteBalance.swift`, NOT built, NOT device-tested. **The Build 30 Swift port is a separate gated session** (port = ~6 constant changes + the warm/cool split + spread term + sCMax in the sC computation; estimator untouched).

**Notes:** bench artifacts this session: `measure_any.swift`+bin (arg-taking baseline), `sweep.swift`+bin (parameterized sweep), `render_cand.swift`+bin (survivor renderer), `sweep1_output.txt`, `cand_A/B/C_log.txt`, 30 `cand_*_*.png` renders (RAW-on-sage | candidate-on-sage, #E8E4CE). Session 26 `MEASURE_COLOR_MODE` flag remains in code and MUST be removed before any App Store promotion. cache 2,510; Build 29 / v1.0.5 LIVE.
